...
class MessageReactionAddDispatcher {

  static async dispatch(messageReaction: MessageReaction, user: User) {
    try {
      await new MessageReactionAddDispatcher(messageReaction, user).parseAndDispatchMessageReaction();
    } catch (e) {
      console.log(e);
    }
  }
  ...
  private async parseAndDispatchMessageReaction() {
    new ReactionRoleManager(this._messageReaction, this._user).filter();
  }
}

export default MessageReactionAddDispatcher;